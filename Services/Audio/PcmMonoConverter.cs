...
public sealed class PcmMonoConverter
{
    ...
    public PcmMonoConverter(WaveFormat sourceFormat, int targetSampleRate, double gain = 1, bool noiseGateEnabled = false)
    {
    ...
    public byte[] Convert(byte[] buffer, int bytesRecorded)
    {
        ...
        var ratio = (double)_sourceFormat.SampleRate / _targetSampleRate;
    ...
    private void AppendMonoSamples(byte[] buffer, int bytesRecorded)
    {
        ...
        for (var offset = 0; offset + frameSize <= bytesRecorded; offset += frameSize)
        {
            ...
            {
                mixed += ReadSample(buffer, offset + channel * bytesPerSample);
            }
